...
function Convert-MarkdownFileToDocx {
    ...
    $docxPath = [System.IO.Path]::ChangeExtension($MarkdownPath, '.docx')
    $htmlPath = [System.IO.Path]::ChangeExtension($MarkdownPath, '.html')
        ...
        if ($line.StartsWith('```')) {
            if (-not $inCode) {
                ...
                if ($inMermaid) {
                    ...
                    $docDir = Split-Path -Parent $MarkdownPath
                    $imgPath = Join-Path $docDir $imgName
                    try {
...
Get-ChildItem -Path $baseDir -Filter '*.md' -File |
    Where-Object { $_.Name -ne 'Marco_Operativo_Contrato_Abr-Jun_2026.md' } |
    ForEach-Object { Convert-MarkdownFileToDocx -MarkdownPath $_.FullName }
